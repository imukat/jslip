...
class UserMenuController extends Controller
{
    ...
    private function _menu() {
        ...
        if (empty($this->param['bid'])) {
            if (empty($_SESSION['minfo']['mid'])) {
                $this->dat      = ['err' => 'Out of service.'];
                $this->viewName = 'user_menu_error';
                return;
            } else {
                if ($_SESSION['minfo']['role'] == 'root') {
                    $basic = $this->model->getBasicByBid($_SESSION['minfo']['bid']);
                } else {
                    if (empty($_SESSION['minfo']['bid'])) {
                        $basic = $this->model->getBasicByMid($_SESSION['minfo']['mid']);
                    } else {
                        $basic = $this->model->getBasicByBid($_SESSION['minfo']['bid']);
                    }
                }
        ...
        $this->dat = ['basic' => $basic];

        if (empty($this->dat['basic'])) {
            $this->dat      = ['err' => 'Out of service.'];
        ...
        if (empty($this->param['bid'])) {
            $_SESSION['minfo']['bid'] = $this->dat['basic'][0]['id'];
        } else {
